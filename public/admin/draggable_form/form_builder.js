$(document).ready(function () {
    ...
    function getButtonFieldHTML() {
        ...
        return $('<div>').addClass('li_' + field + ' w-100 form_builder_field').html(html);
    }
    ...
    function getPreview(plain_html = '') {
        ...
        el.each(function () {
            ...
            if (data_type === 'textarea') {
                ...
                html += '<div class="form-group"><label class="control-label">' + label + '</label><textarea rows="5" name="' + name + '" placeholder="' + placeholder + '" class="form-control" ' + required + '/></textarea>';
            }
            if (data_type === 'date') {
                var checkbox = $(this).find('.form-check-input');
                var required = '';
