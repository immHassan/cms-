      ...
      this.each(function() {
        ...
        <div class="imageuploadify well">
          ...
          <div class="imageuploadify-images-list text-center"> 
            <button type="button" class="btn btn-default selectimg d-none">Drag and drop a images here or click to select</button>
          </div>
        </div>
        `);
        ...
        $(self).closest("form").on("submit", function(event) {
          ...
          xhr.onreadystatechange = function(e) {
            if (xhr.status == 200 && xhr.readyState === XMLHttpRequest.DONE) {
                ...
                var finaldata =JSON.parse(xhr.response);
                var count =finaldata.count;
                finaldata=finaldata.data;
                for (var i = 0; i < finaldata.length; i++) {
                    var image = finaldata[i].image
                    var id = finaldata[i].id
                    var create =finaldata[i].created_at
                    
                    let container = $(`
                    <div class='imageuploadify-container hero  this${id}' style="margin-top:10px" id="${image}">
                    <button type='button'class='btn btn-danger' onclick="deletethis(${id})">
                    <i class='fa fa-trash' style="font-size: 15px;margin-top: 0px;"></i>
                    </button>
                    ...
                    container.css("margin-left", 10 + "px"); 
                    ...
                    let imageapp = $("<img>"); 
                    imageapp.attr("src", "/uploads/media/"+image); 
                    container.append(imageapp); 
                    imagesList.prepend(container); 
                    imagesList.find(".imageuploadify-container:nth-child(" + boxesNb + "n+4)").css("margin-left", 10 + "px"); 
                    imagesList.find(".imageuploadify-container:nth-child(" + boxesNb + "n+3)").css("margin-right", 10 + "px");
                
                    $('.image_section').prepend(`<div class="col-sm-3">
                      <div class="hero" id="${image}">
                          <div class="card p-3">

                          <label class="imagecheck mb-3">
                          <input name="imagecheck" type="checkbox"class="multidelete imagecheck-input" id="${id}" value="${id}" />
                          <figure class="imagecheck-figure">
                              <img src="/uploads/media/${image}" alt="${image}" class="rounded imagecheck-image" style="height:150px">
                              ...
                              <div class="d-flex align-items-center px-2">
                                  <div>
                                      <div>${image}</div>
                                      <small class="d-block text-muted">${moment(create).fromNow()}</small>
                                  </div>     
                              </div>
                          </div>
                      </div></div>`);
                    }
                    $('.countimages').html('('+count+') Photos');
                    $('.hidethis').hide();
            }
          }
